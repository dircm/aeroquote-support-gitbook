...
AeroQuote's tax engine currently handles GST (Australia / New Zealand), VAT (Europe / UK / Africa), sales tax (US / Canada) and any other operator-level tax line you need to itemise — in the operator's local currency and using the operator's preferred terminology. Once configured, the breakdown flows automatically onto your customer-facing quote pages, booking pages, PDFs, confirmation emails and downloadable tax invoices.

{% hint style="info" %}
**Tax setup is optional,** if you do nothing AeroQuote will continue to operate. If you do not confgure any tax items, your customer-facing pricing is unchanged — totals show exactly the amount you entered into AeroQuote, with no changes.
{% endhint %}
...
There are two parts to tax setup, and they live on the same Tax page:

| Setting                 | What it does                                                                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tax invoice details** | Legal business name, address, registration label (ABN / VAT / EIN), registration number — these appear on the tax invoice header            |
| **Tax rates & items**   | Per-rate name, percentage, inclusive flag, scope (fare / surcharge / add-ons) — drives the itemised lines on the customer page and receipts |

Tax rates apply app-wide — the same configuration drives charter quotes / bookings, scheduled-flight ticket purchases, and tour-package bookings.

<figure><img src="../../.gitbook/assets/tax.jpg" alt=""><figcaption></figcaption></figure>

***

## Step 1 — Set your tax rates
...
| **Is inclusive** | Whether the rate is already baked into the fares you enter (inclusive) or added on top (exclusive) — see [Inclusive vs exclusive](tax-setup.md#inclusive-vs-exclusive) below |
...
## Step 2 — Set your tax invoice details
...
2. Fill in the four tax invoice fields:
...
Tax invoice details panel has a **Preview** button. It opens a mock invoice built from your current details and a sample fare run through your tax items — so you can see exactly how the header flips between "Tax invoice" and "Receipt" and how the breakdown itemises, before saving. The preview reflects unsaved edits, so tweak a field and re-open it to check the result.
...
1. Go to **Customer Pages Branding**
2. Find the **Tax invoice / receipt download (bookings only)** checkbox
3. Uncheck it and click **Save branding**

<figure><img src="../../.gitbook/assets/tax setting.jpg" alt=""><figcaption></figcaption></figure>
...
{% hint style="success" %}
**Run a test quote after configuring tax** — Create a test quote and check that the customer page, PDF, and confirmation email all render the breakdown you expect. The customer page is the fastest preview while you're tuning rates.
{% endhint %}
...
* If Xero or QuickBooks is already producing your formal invoices, hide the AeroQuote receipt download in Customer Pages Branding so customers aren't offered two competing documents — see [Hiding the receipt download](tax-setup.md#hiding-the-receipt-download).
